...
public class ArtifactStoreService extends PluginProfilesService<ArtifactStore> {
    ...
    public Map<String, String> artifactStoreIdsToPluginIds() {
        HashMap<String, String> storeIdsToPluginIds = new HashMap<>();
        for (ArtifactStore artifactStore : getPluginProfiles()) {
            storeIdsToPluginIds.put(artifactStore.getId(), artifactStore.getPluginId());
        }
        return storeIdsToPluginIds;
    }
}
